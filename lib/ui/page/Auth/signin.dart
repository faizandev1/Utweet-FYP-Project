...
class _SignInState extends State<SignIn> {
  ...
  void _emailLogin() {
    ...
    final isValid = Utility.validateCredentials(
      context,
    ...
    if (isValid) {
      authState
        ...
        final userEmail = authState.user?.email?.toLowerCase() ?? '';
        if (!userEmail.endsWith('@student.uol.edu.pk')) {
          Utility.customSnackBar(
            context,
            'Only @student.uol.edu.pk emails are allowed to login.',
          );
          authState.logoutCallback.call(); // Optional logout
          return;
        }
