...
class FeedState extends AppState {
  ...
  /// -=-=================-=-==-======================-=-=-===================-=-
  handleLocationPermission(String userID) async {
    // Show.showLoader();
    ...
    if (!serviceEnabled) {
      ...
      Show.showErrorSnackBar('Error', 'Location services are disabled. Please enable the services');
    ...
    if (permission == LocationPermission.denied) {
      // Get.isOverlaysOpen ? Get.back(): null;
      permission = await Geolocator.requestPermission();
      if (permission == LocationPermission.denied) {
        // Get.isOverlaysOpen ? Get.back(): null;
        print('Location permissions are denied');
        Show.showErrorSnackBar('Error', 'Location permissions are denied');
    ...
    if (permission == LocationPermission.deniedForever) {
      // Get.isOverlaysOpen ? Get.back(): null;
      Show.showErrorSnackBar('Error', 'Location permissions are permanently denied, we cannot request permissions.');
      print('Location permissions are permanently denied, we cannot request permissions.');
      return false;
    }

    await getCurrentLocation(userID);
  ...
  getCurrentLocation(String userID) async {
  ...
  Future<void> updateUserLocation({
    ...
    FirebaseDatabase.instance.ref().child('profile').child(userId);
  ...
  Future<List<UserModel>> fetchAllUsersFromRealtimeDB() async {
    try {
      final DatabaseReference dbRef = FirebaseDatabase.instance.ref().child('profile');
